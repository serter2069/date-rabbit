...
export class EmailWorker {
  ...
  @Process()
  async handle(job: { data: EmailNotificationJob }): Promise<void> {
    const { notificationId, title, body, recipientEmail, deduplicationKey } = job.data;
    ...
    try {
      ...
      this.logger.error(`Email worker failed for notification ${notificationId}: ${error}`);
