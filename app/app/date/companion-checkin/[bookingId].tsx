...
export default function CompanionCheckinScreen() {
  ...
  // Poll every 5 seconds after companion checked in
  useEffect(() => {
    if (!checkedIn) return;
  ...
  const handleCheckin = async () => {
    setCheckingIn(true);
    try {
      const updated = await activeDateApi.companionCheckin(bookingId);
      setBooking(updated);
    ...
    <View style={styles.container}>
      ...
      {!checkedIn ? (
        ...
        <View style={styles.waitingCard}>
          ...
          <Text style={styles.waitingSubtext}>Waiting for seeker to check in...</Text>
      ...
      {booking && (
        <View style={styles.statusRow}>
          <View style={[styles.dot, booking.companionCheckinAt ? styles.dotGreen : styles.dotGray]} />
          <Text style={styles.statusName}>You</Text>
          <View style={[styles.dot, booking.seekerCheckinAt ? styles.dotGreen : styles.dotGray, { marginLeft: 24 }]} />
          <Text style={styles.statusName}>{booking.seeker?.name ?? 'Seeker'}</Text>
        </View>
      )}
    </View>
  );
